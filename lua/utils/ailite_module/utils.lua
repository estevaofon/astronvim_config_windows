...
function M.estimate_tokens(text)
  local config = require("utils.ailite_module.config").get()
...
-- Get file extension
function M.get_file_extension(filepath) return vim.fn.fnamemodify(filepath, ":e") end
...
-- Join lines
function M.join_lines(lines) return table.concat(lines, "\n") end

-- Trim whitespace
function M.trim(str) return str:gsub("^%s+", ""):gsub("%s+$", "") end

-- Format timestamp
function M.get_timestamp() return os.date "%H:%M:%S" end

-- Show notification with formatted message
function M.notify(message, level) vim.notify(message, level or vim.log.levels.INFO) end
