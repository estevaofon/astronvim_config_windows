...
function M.add_file(filepath)
  ...
  for _, existing_file in ipairs(M.plugin.selected_files) do
    if existing_file == filepath then return false end
  end

  table.insert(M.plugin.selected_files, filepath)

  -- Limpa arquivos enviados ao adicionar novo arquivo
  M.clear_sent_files()
  return true
end
...
function M.clear_files() M.plugin.selected_files = {} end

-- Chat history management
function M.add_to_history(role, content) table.insert(M.plugin.chat_history, { role = role, content = content }) end

function M.clear_history() M.plugin.chat_history = {} end
...
function M.next_code_block()
  if #M.plugin.code_blocks == 0 then return nil end
...
function M.prev_code_block()
  ...
  if M.plugin.current_code_block < 1 then M.plugin.current_code_block = #M.plugin.code_blocks end
  return M.plugin.current_code_block
end
...
function M.get_unseen_files()
  ...
  for _, f in ipairs(M.plugin.selected_files) do
    ...
    if not sent_set[norm] then table.insert(unseen, norm) end
  end
  return unseen
end
...
function M.clear_sent_files() M.plugin.sent_files = {} end

-- Buffer/Window validation helpers
function M.is_chat_valid() return M.plugin.chat_buf and vim.api.nvim_buf_is_valid(M.plugin.chat_buf) end

function M.is_chat_win_valid() return M.plugin.chat_win and vim.api.nvim_win_is_valid(M.plugin.chat_win) end
